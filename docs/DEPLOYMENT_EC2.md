...
- Outbound HTTPS must be allowed (Neon, Brave Search, business websites).
...
Use the Neon **pooled** connection string from the project dashboard
(Connection Details → Pooled connection). Verify with:
...
Apply new migrations from your machine with `npm run db:migrate` in `apps/web`
